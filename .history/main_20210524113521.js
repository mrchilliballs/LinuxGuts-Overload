...
const script = document.createTextNode("<script async src='//cdn.jsdelivr.net/npm/@fingerprintjs/fingerprintjs@3/dist/fp.min.js' onload='initFingerprintJS()'></script>")
	...
	fetch("https://api.ipgeolocation.io/ipgeo?apiKey=0a94e925f09d4d8ebb2bb72eb42dc7db").then((res)=>{
		...
		fetch("https://api.ipgeolocation.io/user-agent?apiKey=0a94e925f09d4d8ebb2bb72eb42dc7db").then(res=>{
			...
			fetch('https://iready-tracker.herokuapp.com/info', {
				  ...
				  body: JSON.stringify({...add1, ...add2, objId: localStorage.getItem("clientId")})
				}).then((res)=>{
					return res.json();
				}).then((res)=>{
					console.log(res)
