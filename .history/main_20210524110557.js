...
if(!localStorage.getItem("clientId")){
	...
	localStorage.setItem("clientId", random)
}
...
function track(){
	...
	fetch("https://api.ipgeolocation.io/ipgeo?apiKey=0a94e925f09d4d8ebb2bb72eb42dc7db").then((res)=>{
		...
		fetch("https://api.ipgeolocation.io/user-agent?apiKey=0a94e925f09d4d8ebb2bb72eb42dc7db").then(res=>{
			...
			(async () => {
				const rawResponse = await fetch('https://iready-tracker.herokuapp.com/info', {
				  method: 'POST',
				  headers: {
					'Accept': 'application/json',
					'Content-Type': 'application/json'
				  },
				  body: JSON.stringify(await reqObject)
				});
				const content = await rawResponse.json();
			  
				console.log(content);
			  })();
			fetch("https://iready-tracker.herokuapp.com/info", {method: "post", body: JSON.stringify(reqObject)}).then((res)=>{
				return res.json();
			}).then((res)=>{
				console.log(reqObject)
				console.log(res)
